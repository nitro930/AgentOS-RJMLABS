...
const helpData: HelpArticle[] = [
  ...
  },
  {
    ...
    layer: 'L3',
    category: 'Core Layers',
    description: 'Configure and route AI model requests to different LLM providers (OpenAI, Anthropic, Google, local models). The Brain Router handles load balancing, failover, token counting, and cost attribution per model.',
    howToUse: [
      'Open Brain Router from the sidebar to see all configured model providers and their status.',
      'Click the "Add Provider" button to register a new LLM endpoint (e.g., OpenAI GPT-4o, Anthropic Claude 3.5).',
      'Set routing rules: map agent types or task complexity to specific models (e.g., simple tasks → GPT-4o-mini, complex → Claude 3.5 Sonnet).',
      'Monitor the usage chart showing token consumption and cost per model over the last 7 days.',
      'Click "Test Connection" on any provider card to verify API key validity and latency.',
    ],
    howToSetUp: [
      'Navigate to Brain Router and click "Add Provider". Enter your API key in the Security Vault first, then reference it here.',
      'For each provider, configure the default model, max tokens, temperature, and rate limits.',
      'Set up failover chains: if the primary model returns a 429/500, the router falls back to the next provider in the chain.',
      'Enable token tracking in Settings → Brain → Track Usage to populate the cost charts.',
      'Add local model endpoints (Ollama, vLLM) by entering the base URL and selecting "Local" as the provider type.',
    ],
    tips: [
      'Use GPT-4o-mini for high-volume, low-complexity tasks to keep costs down — it\'s 60x cheaper than GPT-4o.',
      'Set up a failover chain of at least 2 providers to avoid single-point-of-failure outages.',
      'Monitor the Token Usage chart weekly to identify agents that are burning through context windows unnecessarily.',
    ],
  },
  {
    id: 'agents',
    ...
    howToSetUp: [
      'Ensure Brain Router has at least one model provider configured before creating agents.',
  ...
  },
  {
    ...
    layer: 'SYS',
    category: 'System',
    description: 'First-run setup wizard that guides new users through initial AgentOS configuration. Connect model providers, create your first agent, test it in the playground, and deploy to production.',
    howToUse: [
      'Onboarding launches automatically on first login. You can also access it from the System section of the sidebar.',
      'Follow the step-by-step wizard: Step 1 — Add your first API key (OpenAI or Anthropic) to Security Vault.',
      'Step 2 — Configure Brain Router with your API key and select a default model.',
      'Step 3 — Create your first agent using the guided agent builder.',
      'Step 4 — Test your agent in the Playground, then click "Complete Onboarding" to finish.',
    ],
    howToSetUp: [
      'Customise the onboarding wizard in Settings → Onboarding → Steps to match your organisation\'s workflow.',
      'Pre-configure default values (model, timezone, currency) so users have fewer choices to make.',
      'Enable "Skip Onboarding" for experienced users who want to configure manually.',
      'Set onboarding as required for new users in Settings → Onboarding → Require Completion.',
      'Configure onboarding completion tracking to see how many users have completed the setup.',
    ],
    tips: [
      'Complete the onboarding wizard even if you\'re experienced — it sets up important defaults.',
      'You can always re-run onboarding from the sidebar if you skipped it initially.',

...
const helpData: HelpArticle[] = [
  ...
  },
  {
    ...
    layer: 'L4',
    category: 'Core Layers',
    description: 'Create, configure, and manage AI agents with a library of 50+ pre-built templates. Each agent has a system prompt, model assignment, skill set, memory scope, and runtime parameters. The template library covers Development, Security, DevOps, Content & Marketing, Data & Analytics, Business & Finance, Support, Research, Design, Compliance, and AI/ML — each with expert-crafted system prompts ready to use.',
    howToUse: [
      'Open the Agents grid to see all agents displayed as cards with status, model, and last activity.',
      'Click "New Agent" to open the agent builder with two modes: Quick Start (template library) and Custom (build from scratch).',
      'In Quick Start mode, browse 50+ pre-built templates organised by category. Use the search bar to find templates by keyword, or filter by category (Development, Security, DevOps, Content, Data, Business, Support, Research, Design, Compliance, AI/ML). Each template includes a specialist system prompt pre-configured for that role.',
      'Click any template card to select it — the name, prompt, model, avatar, and colour are auto-filled. Switch to Custom mode to fine-tune the prompt or settings before creating the agent.',
      'Click any existing agent card to open its detail view: edit the prompt, view conversation history, check costs, and see associated workflows.',
    ...
    howToSetUp: [
      ...
      'Use Quick Start templates to get agents running fast — each template has an expert-crafted system prompt, so you don\'t need to write one from scratch. Pick a template, optionally customise it, and create the agent in seconds.',
      'Template categories include: Development (Code Reviewer, Bug Hunter, API Architect, Test Engineer, Frontend/Backend/Full Stack Dev, Refactoring, Mobile Dev, Database Architect), Security (Security Scanner, Pen Tester, Compliance Auditor, Incident Responder, Threat Intelligence), DevOps (DevOps Automator, Cloud Architect, SRE Engineer, Kubernetes Expert, Terraform Specialist, Monitoring Expert), Content & Marketing (Content Writer, Technical Writer, Social Media Manager, Copy Editor, Brand Strategist, SEO Specialist, Email Marketer), Data & Analytics (Data Analyst, SQL Expert, Data Engineer, BI Analyst, Statistical Analyst, Python Data Scientist), Business & Finance (Financial Analyst, Market Analyst, Project Manager, Product Manager, Startup Advisor, Risk Analyst, Sales Strategist), Support (Customer Support, Email Composer, Chatbot Designer, Knowledge Base Manager, Community Manager), Research & Education (Academic Researcher, Trend Scout, Patent Analyst, Curriculum Designer, Fact Checker, Science Writer), Design & Creative (UX Designer, UI Component Builder, Design System Architect, Illustration Prompter, Motion Designer), Compliance & Legal (Legal Advisor, Privacy Officer, Accessibility Auditor, Contract Analyst), and AI & Machine Learning (ML Engineer, Prompt Engineer, RAG Specialist, AI Ethics Reviewer, LLM Fine-tuner, AI Agent Architect).',
      'When creating a custom agent, write a clear and specific system prompt — include role definition, output format, constraints, and examples.',
    ...
    tips: [
      ...
      'Use the template search to quickly find relevant agents — type "security" to see all security-focused templates, or "content" for writing and marketing agents.',
      'Template prompts are starting points — don\'t hesitate to customise them for your specific use case after selecting a template.',
      'For best results with financial or business agents, ensure all values are specified in GBP (£) to match the platform\'s currency settings.',
    ],
  },
  {
    id: 'workflows',
  ...
  },
  {
    ...
    layer: 'L1',
    category: 'Tools & Integrations',
    description: 'Browser-based SSH terminal for direct access to your VPS. Execute commands, manage services, edit files, and debug issues without leaving the AgentOS interface. Includes 60+ quick commands across 9 categories, command history navigation, favourites, and output export.',
    howToUse: [
      ...
      'Quick Commands panel below the terminal provides 60+ one-click commands across 9 categories: System, CPU & Memory, Disk & Storage, Network, Docker, Security, Processes & Logs, Git & Dev, and Deploy.',
      'Use the 🚀 Full Deploy quick command to pull the latest code from GitHub and rebuild the Docker container in one step — perfect after making changes.',
  ...
  },
  {
    ...
    layer: 'L5+',
    category: 'Tools & Integrations',
    description: 'A visual task management board with drag-and-drop cards organised into columns representing workflow stages. The Kanban Board shows agent tasks across four columns — To Do, In Progress, Reviewing, and Completed — with priority badges, agent assignments, and colour-coded priority indicators so you can see the status of all agent work at a glance.',
    howToUse: [
      'Open Kanban Board from the Tools & Integrations section of the sidebar — you\'ll see four columns: To Do, In Progress, Reviewing, and Completed.',
      'Each card shows the task title, assigned agent avatar, priority badge (Critical=red, High=orange, Medium=yellow, Low=green), and tags.',
      'Drag and drop cards between columns to change their status. For example, drag a card from "To Do" to "In Progress" when an agent starts working on it.',
      'Click the "+" button at the top of any column to add a new task to that column. Enter a title, description, assign an agent, set priority, and add tags.',
      'Use the Filter bar at the top to filter tasks by agent, priority, or tag. This is especially useful when you have many agents and need to focus on a specific one.',
      'Click any task card to expand its details: view the full description, edit the task, change the assigned agent, or add comments.',
    ],
    howToSetUp: [
      'The Kanban Board works out of the box with demo data showing sample tasks across all four columns.',
      'Customise column names and add new columns in Settings → Kanban → Columns if your workflow has different stages (e.g., "Backlog", "Testing", "Deployed").',
      'Set up WIP (Work In Progress) limits per column to prevent bottlenecks — e.g., limit "In Progress" to 5 tasks so agents don\'t take on too much at once.',
      'Integrate with Agent Teams so that when a team is assigned to a task, all team members appear on the card.',
      'Enable automatic status transitions in Settings → Kanban → Automation — e.g., move a card to "Reviewing" when an agent marks its output as ready for review.',
    ],
    tips: [
      'Use the Kanban Board alongside the Activity Timeline — the Board shows current state, while the Timeline shows what changed and when.',
      'Set WIP limits on the "In Progress" column to prevent agents from being overwhelmed with too many concurrent tasks.',
      'Colour-coded priority badges let you quickly spot critical tasks — address Critical and High priority items first.',
      'Use tags to group related tasks (e.g., "sprint-1", "bugfix", "feature") so you can filter the board by tag.',
    ],
  },
  {
    id: 'dependency-graph',
    ...
    layer: 'L4++',
    category: 'Tools & Integrations',
    description: 'A visual network diagram showing the relationships and dependencies between agents. The Dependency Graph displays agents as nodes and their delegation, chain, and team connections as directed edges, helping you understand the flow of work across your agent ecosystem and identify critical dependency chains or single points of failure.',
    howToUse: [
      'Open Dependencies from the Tools & Integrations section of the sidebar — you\'ll see the graph view with agents as circular nodes and connections as directed arrows.',
      'Agent nodes are colour-coded by status: green = active, yellow = idle, red = error, grey = paused. Node size reflects the number of connections — larger nodes are more connected.',
      'Hover over any agent node to see a tooltip with the agent name, status, number of dependencies, and what it connects to.',
      'Click any agent node to highlight its direct connections — upstream dependencies (agents it depends on) and downstream dependents (agents that depend on it).',
      'Use the "Select Agent" dropdown at the top to focus on a specific agent and its immediate neighbourhood in the graph.',
      'Use the Layout toggle to switch between force-directed (organic) and hierarchical (top-down) layouts depending on your preference.',
    ],
    howToSetUp: [
      'The Dependency Graph is auto-generated from your agent configuration — it reads delegation rules, chain definitions, and team memberships to build the graph automatically.',
      'Ensure your agents have properly configured delegation targets in the Delegation section — these form the edges of the graph.',
      'Add Agent Chains to create sequential dependencies — each chain step creates a directed edge between agents.',
      'Set up Agent Teams to create group dependencies — team membership creates edges between the leader and member agents.',
      'The graph updates in real-time when you add, remove, or modify agents, chains, or delegation rules.',
    ],
    tips: [
      'Look for single points of failure — if a node has many downstream dependents and goes down, it can cascade across your entire agent network.',
      'Use the hierarchical layout to see the flow of work from top (input agents) to bottom (output agents).',
      'Click on a highly-connected node to audit whether all those connections are necessary — over-connected agents can become bottlenecks.',
      'Combine the Dependency Graph with the Kanban Board to trace a task from its current status back to the agent dependencies that support it.',
    ],
  },
  {
    ...
    layer: 'L8+',
    category: 'Tools & Integrations',
    description: 'A chronological feed of all agent and system events across your AgentOS platform. The Activity Timeline captures every significant action — agent creations, task completions, workflow runs, errors, deployments, and more — with timestamps, severity levels, and detailed context so you can audit, troubleshoot, and understand the full history of your platform.',
    howToUse: [
      'Open Activity from the Tools & Integrations section of the sidebar — you\'ll see a reverse-chronological feed of events with the most recent at the top.',
      'Each event card shows a timestamp, severity icon (info=blue, success=green, warning=amber, error=red), event title, description, and the source agent or system component.',
      'Use the Filter bar to narrow events by type (Agent, System, Workflow, Error), severity level, or date range.',
      'Click the Search bar to search events by keyword — useful for finding specific error messages or agent actions.',
      'Click any event card to expand it and see the full details including associated agent ID, workflow ID, and any related events.',
      'Use the date range picker to focus on a specific period — e.g., "show me all errors from the last 24 hours".',
    ],
    howToSetUp: [
      'The Activity Timeline captures events automatically — no configuration needed for basic logging.',
      'Configure which event types are logged in Settings → Activity → Event Types — enable or disable specific categories like agent_lifecycle, task_completion, workflow_execution, system_alert, and error.',
      'Set the event retention period in Settings → Activity → Retention (default: 90 days). Older events are automatically purged.',
      'Enable real-time streaming via WebSocket for live event updates without refreshing the page.',
      'Set up Alert Channel integrations for critical events — e.g., send an email when any error-level event is logged.',
    ],
    tips: [
      'Check the Activity Timeline first when troubleshooting — it shows the exact sequence of events that led to an issue.',
  ...
  },
  {
    ...
    layer: 'L4+',
    category: 'Tools & Integrations',
    description: 'Multi-agent conversation spaces with threaded discussions, reactions, and pinned messages. Chat Rooms provide dedicated spaces where agents and users can collaborate in real-time, share context, and coordinate on tasks together. Support for group, direct, broadcast, and threaded room types.',
    howToUse: [
      'Open Chat Rooms from the Tools & Integrations section of the sidebar — you\'ll see 4 tabs: Rooms, Messages, Members, and Settings.',
      'Rooms tab: Browse all chat rooms with member count, last message preview, and unread badges. Use the search bar to filter rooms. Click "Create Room" to make a new room with a name, type (Group, Direct, Broadcast, or Threaded), and description.',
      'Messages tab: Select a room to see its conversation. Type messages in the input box and press Enter or click Send. Use the thread reply button on any message to start a threaded discussion. Add emoji reactions by clicking the smile icon. Pin important messages using the pin button. Code blocks are rendered with syntax highlighting — wrap code in backticks.',
      'Members tab: View all room members with their online status (green=online, yellow=away, red=busy, grey=offline) and roles (Admin, Moderator, Member, Observer). Agents are marked with a BOT badge.',
      'Settings tab: Configure room notifications (push, sounds, presence), toggle compact mode, and manage room privacy settings.',
    ],
    howToSetUp: [
      'Chat Rooms work out of the box with demo data — create your first room by clicking the "Create Room" button.',
      'Choose the room type based on your needs: Group rooms for team discussions, Direct for 1-to-1 conversations, Broadcast for announcements from one to many, and Threaded for organised discussions with reply threads.',
      'Add agents to rooms by using the Members tab — agents will automatically receive messages and can respond based on their configured prompts.',
      'For best results, give each agent a clear role in the room — e.g., "You are the code reviewer in this room. Respond to code snippets with constructive feedback."',
      'Pin key messages (decisions, action items) so new members can quickly get up to speed when joining a room.',
    ],
    tips: [
      'Use threaded replies to keep conversations organised — start a thread for each sub-topic rather than mixing everything in the main channel.',
      'Broadcast rooms are ideal for system announcements — only admins can post, and all members receive the message.',
      'Use emoji reactions for quick feedback — agents can be instructed to react with specific emojis to indicate agreement, completion, or flags.',
      'Create separate rooms for separate projects to keep context clean and avoid information overload.',
    ],
  },
  {
    id: 'calendar',
    ...
    layer: 'L5+',
    category: 'Tools & Integrations',
    description: 'A visual calendar view of scheduled tasks, meetings, deployments, and deadlines. The Scheduling Calendar provides month, week, and day views with colour-coded events, agent assignment tracking, and a sidebar with a mini calendar, upcoming events, and filters.',
    howToUse: [
      'Open Scheduling Calendar from the Tools & Integrations section of the sidebar.',
      'Switch between Month, Week, and Day views using the tabs at the top. Navigate between periods using the Previous/Next arrows.',
      'Month view shows a full calendar grid with colour-coded event chips. Click any day to drill down. Events that overflow show a "+N more" indicator.',
      'Week view shows a 7-day timeline with hourly slots (6 AM–10 PM). Events are positioned and sized based on their start time and duration.',
      'Day view shows a detailed hourly schedule with a live red current-time indicator line.',
      'Click any event to open a slide-over detail panel showing the event description, assigned agent, status, priority, and options to change status or delete.',
      'Use the sidebar mini calendar for quick navigation, the Upcoming Events list to see what\'s next, and the Agent/Type filters to focus on specific events.',
    ],
    howToSetUp: [
      'The Calendar works out of the box with demo data showing 17+ events across the current month.',
      'Create new events by clicking the "+" button — specify the title, type (Task, Meeting, Maintenance, Deployment, Review, Deadline), start/end time, assigned agent, priority, and recurrence pattern.',
      'Set up recurring events using the recurrence configuration: daily, weekly, monthly, or custom intervals with an optional end date.',
      'Assign agents to events so you can filter the calendar by agent and see each agent\'s workload at a glance.',
      'Configure event reminders (e.g., "15 minutes before") to get notifications before important events via your configured Alert Channels.',
    ],
    tips: [
      'Use the Agent filter in the sidebar to check if an agent is overloaded with too many events on the same day.',
      'Colour-coded event types help you quickly identify patterns — deployment events in red, reviews in green, deadlines in yellow.',
      'Set up recurring maintenance windows as recurring events so agents know when systems will be unavailable.',
      'Use the Week view for planning — it gives the best balance of detail and overview for scheduling agent work.',
    ],
  },
  {
    ...
    layer: 'SYS',
    category: 'Tools & Integrations',
    description: 'A drag-and-drop widget builder for creating personalised dashboard views. Choose from 11 widget types (Stat Cards, Charts, Agent Lists, Task Tables, Gauges, Timelines, Calendars, Agent Status, Cost Trackers, Activity Feeds, and Custom HTML) and arrange them on a responsive grid. Multiple dashboards can be created and switched between.',
    howToUse: [
      'Open Custom Dashboards from the Tools & Integrations section of the sidebar.',
      'Use the dashboard selector at the top to switch between dashboards (e.g., "Operations Overview" and "Agent Performance"). Click "+" to create a new dashboard.',
      'Toggle the Widget Library panel on the left to browse 11 widget types. Click a widget type to add it to the current dashboard, or drag it onto the grid.',
      'Each widget card has a title bar with configure, collapse, and remove buttons (visible on hover). Click the gear icon to open the widget configuration dialog where you can set the title, data source, refresh rate, and colour theme.',
      'Use the Dashboard Toolbar for quick actions: Add Widget, toggle Layout (Grid/Freeform), Save, Reset, Share, and Fullscreen.',
      'Widgets render different content based on type: Stat Cards show a big number with trend, Charts show mini bar/line visualisations, Gauges show circular SVG progress indicators, and so on.',
    ],
    howToSetUp: [
      'Custom Dashboards come with 2 pre-built demo dashboards: "Operations Overview" (8 widgets) and "Agent Performance" (7 widgets).',
      'Start by modifying an existing dashboard — add, remove, or rearrange widgets to suit your needs. Then save the layout.',
      'Create a new dashboard for each perspective: one for operations, one for finance, one for agent performance, etc.',
      'Configure each widget\'s data source to pull from real API endpoints. Set a refresh rate (e.g., 30 seconds) for live-updating widgets.',
      'Use the Share button to make a dashboard public — other users on the same AgentOS instance can view (but not edit) shared dashboards.',
    ],
    tips: [
      'Start with the pre-built dashboards and customise them rather than building from scratch.',
      'Use the Gauge widget for KPIs that have a target (e.g., uptime %, goal completion) and Stat Cards for simple counts.',
      'Set different refresh rates per widget — real-time metrics need 10-30 second refresh, while daily summaries can refresh every 5 minutes.',
      'Create a dedicated "War Room" dashboard with only critical widgets for incident response scenarios.',
    ],
  },
  {
    ...
    layer: 'L8+',
    category: 'Tools & Integrations',
    description: 'Generate and view weekly, monthly, or quarterly PDF-style reports on agent productivity. Reports include an AI-generated executive summary, performance score (0-100), key metrics, agent breakdown, cost analysis in GBP, trend comparisons, and AI recommendations for improvement.',
    howToUse: [
      'Open Performance Reports from the Tools & Integrations section of the sidebar — you\'ll see 3 tabs: Reports, Generate, and History.',
      'Reports tab: Browse generated reports as cards showing the period, type badge, score, and key stats. Hover for a quick preview, click to open the full report.',
      'Full report view includes: Executive Summary (AI-generated text), Performance Score (circular SVG gauge), Key Metrics grid (tasks completed/failed, avg duration, total cost in £, tokens, uptime %), Agent Breakdown table with sparkline trends, Cost Breakdown by agent and model, Trend Comparison vs previous period, and AI Recommendations.',
      'Generate tab: Create a new report by selecting the type (Daily, Weekly, Monthly, Quarterly, Custom), date range, target agents, and which sections to include. Toggle auto-generate to schedule future reports.',
      'History tab: View all past report generation runs with their status, scores, and timestamps.',
    ],
    howToSetUp: [
      'Performance Reports work with existing agent data — no additional setup required for basic reports.',
      'For the most accurate reports, ensure Cost Tracker is configured with up-to-date pricing for your model providers.',
      'Set up auto-generated weekly reports to maintain a consistent performance tracking cadence.',
      'Configure report recipients in Settings → Reports → Recipients to automatically send generated reports to stakeholders via email or Slack.',
      'Use the Benchmarking feature alongside Performance Reports to compare agent performance against industry standards.',
    ],
    tips: [
      'Review the AI Recommendations section first — it highlights the most impactful improvements you can make.',
      'Compare weekly reports to identify trends — a gradual decline in agent performance may indicate prompt drift or model degradation.',
      'Focus on the cost breakdown to identify expensive agents that may need optimisation — sometimes a cheaper model with a better prompt outperforms an expensive one.',
      'Use the Agent Breakdown table to identify underperforming agents that may need prompt revision or model upgrade.',
      'Set up quarterly reports for executive stakeholders — they provide a high-level view without the noise of daily fluctuations.',
    ],
  },
  {
    ...
    layer: 'L4+',
    category: 'Tools & Integrations',
    description: 'Pre-built workflow patterns that users can instantiate with one click. The template library includes 10+ built-in patterns (Code Review Pipeline, Content Generation, Daily Report, Deployment Gate, Incident Response, Data ETL, Customer Support, Monitoring Alert, Multi-Model Research, Backup & Sync) covering common automation scenarios. Users can also create and share custom templates.',
    howToUse: [
      'Open Workflow Templates from the Tools & Integrations section of the sidebar — you\'ll see 3 tabs: Browse, My Templates, and Builder.',
      'Browse tab: Explore the template library with search, category filter (Automation, Data Pipeline, Agent Chain, Monitoring, Deployment, Custom), and difficulty filter (Beginner, Intermediate, Advanced). Each template card shows the icon, name, description, author, version, difficulty, required agents, estimated setup time, use count, and rating.',
      'Click "Use Template" on any card to instantiate it — this creates a new Workflow in the Workflow Builder pre-configured with the template\'s steps and connections. The template\'s use count increments automatically.',
      'Click a template card to open the detail drawer showing the full step-by-step flow visualisation (vertical pipeline), required variables, trigger configuration, and an "Instantiate" button.',
      'My Templates tab: View and manage your custom templates. Duplicate, preview, edit, delete, or use them.',
      'Builder tab: Create a new template from scratch. Add steps (Agent Call, Condition, Transform, Delay, Webhook), configure each step\'s settings, define variables, set the trigger type, and preview the flow before saving.',
    ],
    howToSetUp: [
      'Workflow Templates are available out of the box with 10 pre-built templates — no setup required.',
      'Start by browsing the built-in templates and instantiating one to see how it works. Each template creates a full Workflow with all steps configured.',
      'For custom templates, use the Builder tab. Define each step clearly: what agent to call, what input to pass, and what to do with the output.',
      'Add variables to your templates so they can be customised at instantiation time — e.g., a "repository URL" variable in a Code Review Pipeline template.',
      'Share useful templates with your team by setting the template visibility to Public — other users on the same AgentOS instance can browse and use it.',
    ],
    tips: [
      'Start with the built-in templates and modify them for your needs rather than building from scratch — they represent proven patterns.',
      'The "Code Review Pipeline" template is a great starting point for teams that want to automate their code review process.',
      'Use the "Daily Report" template as a foundation for scheduled reporting workflows — just customise the data sources and delivery channel.',
      'When creating custom templates, include clear descriptions and tags so other users can discover and understand them.',
